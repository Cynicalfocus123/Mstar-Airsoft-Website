...
- Force of Conquest Event Info includes a bilingual Event Map section directly below Event Information. It now uses the updated uploaded `map final edit.png` image, copied as `force-of-conquest-event-map-updated.png`, replacing the old Event Map image. The map stays full responsive width with automatic height, centered directly on the dark tactical background without a card, panel, heavy border, cropping, stretching, or horizontal overflow. No cPanel ZIP was created and no real server upload was performed.
