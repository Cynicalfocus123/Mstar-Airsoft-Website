...
- The 2026-09-12 homepage support strip sits between the ticket hero and the two language video cards. It uses the supplied transparent National Thailand Airsoft and Paintball Association logo at its original 1454px resolution, compact uppercase support copy matched to the video heading scale, a single desktop text line, and safe mobile stacking while matching the existing dark, gold-accented home page. Only the logo is clickable and it links to `https://thaiairsoft.org/`; the CSS no longer adds a second circle around the original mark. No existing section styling or page behavior changed.
...
- The 2026-06-28 clean Hostinger current deployment package pass is delivery-only and does not redesign the site. The package is built from the current root-domain `dist` output through `npm run zip:hostinger-current`, stages only runtime-referenced assets, and preserves real `assets/`, `images/`, `banners/`, and `videos/` folders so Hostinger extraction creates directories instead of backslash filenames. The package includes the current Sponsor page asset set under `images/sponsor/`, including the latest icons and WebP sponsor media, plus the current compiled bundles `index-CwCwwbt0.js` and `index-DsCkN_Vt.css`; stale live bundles are excluded. Staging-folder rendered checks passed desktop/mobile routes, Thai toggles, media loading, and mojibake checks before packaging. No visual layout, Ticket/Stripe behavior, forms, DNS, SSL, cPanel settings, Cloudflare settings, or live upload changed.
- The 2026-06-28 Become a Sponsor repair keeps the page as a clean white/off-white sponsorship prospectus, not a tactical dark page and not a checkerboard surface. The page uses provided local sponsorship icon assets from `site content/sponsorship page` through deploy-safe `public/images/sponsor/icons/` paths, adding only the missing Campaign Reach megaphone icon while preserving already-correct icons. Sponsor cards and left/right spreads now use tighter sponsor-scoped responsive safeguards: wider desktop grids, two-column tablet cards where appropriate, full-width mobile stacking, contained icons, safe text wrapping, rounded cards with top accent bars, clean image frames, visible phase numbers, and a connector line that only appears between desktop marketing-phase cards. A centered premium CTA now closes the page with `mailto:info@mstarairsoft.com?subject=Sponsor`. Existing Sponsor SEO metadata/sitemap coverage was confirmed, so no duplicate SEO logic was added. No cPanel ZIP, live upload, DNS/SSL/cPanel, Stripe, Ticket, form, or `.htaccess` behavior changed.
- The 2026-06-28 latest live-site optimization pass is media/deployment weight work only and does not redesign pages. The Event Info briefing image and Sponsor premium placement collage now use WebP deploy assets at the same visual crop/layout points: `referee-final-edit.webp` and `premium-placement-collage.webp`. The original PNGs were moved outside deploy paths to `site content/non-deployed-heavy-media-backup/latest-heavy-cleanup/`. The Event Map remains PNG for readability. Live `https://mstarairsoft.com` rendered verification passed all requested desktop/mobile routes with Thai toggles, no mojibake, and no missing media; fresh local `dist` verification also passed. The final ZIP is `mstar-airsoft-live-optimized-latest.zip` from `dist` contents only, with no live upload, DNS/SSL/cPanel change, Stripe/payment change, form change, `.htaccess` routing change, or visual redesign.
- The 2026-06-28 GitHub Pages deploy repair is workflow/media-delivery only: Git push was already successful, but Pages failed during Actions checkout because the referenced homepage hero MP4 was stored through Git LFS and Actions could not fetch the LFS object. `public/videos/force-of-conquest-header-compress-video.mp4` now stays as a normal Git-tracked deploy asset by removing `public/videos/*.mp4` from LFS tracking, while `public/media/*.mp4` LFS tracking remains unchanged. This preserves the existing hero video presentation and does not change sponsor layout, page design, Ticket, Stripe/payment controls, event pages, header/footer, DNS/cPanel/.htaccess behavior, deployment ZIP scripts, or live upload behavior.
- The 2026-06-28 Retail & Commercial Zones inline-icon pass keeps `/become-a-sponsor` on the same clean white/off-white slide-deck background while replacing only the three Vendor & Commercial Zones row markers with supplied PNG assets. Premium Retail Hubs uses `vendor-stall-orange.png`, F&B Concessions uses `food-orange.png`, and Technical Services uses `orange-gear.png`; each is aligned in a consistent sponsor-scoped inline grid with object-fit contain, matching size, stable text spacing, and safe desktop/tablet/mobile wrapping. Header, footer, other sponsor sections, Ticket, Stripe/payment controls, event pages, DNS/cPanel/.htaccess behavior, deployment ZIP scripts, and live upload behavior remain unchanged. No dev server, preview port, cPanel ZIP, live upload, or Serena dashboard was used.
- The 2026-06-28 sponsor icon/layout pass keeps `/become-a-sponsor` as a clean off-white slide-deck page with no checker/grid page background and no heavy whole-page boxes. Sponsor pills use supplied transparent PNG icons from the local sponsorship folder: Event Overview uses the magnifier/eye icon, Vendor Program uses the vendor booth icon, Media Inventory uses the media/video icon, and both Sponsorship and Partner Value use the handshake icon. Card icons now use supplied PNG assets for Mobile Assets, Player Kit, Equipment Box, Physical Media Points, Direct Placement Benefits, Premium Visibility/Main Stage, Food & Beverage, Tactical Service, and Core Operations Advantages, with object-fit contain, transparent backgrounds, clean forward-slash public paths, cropped padding, and reduced dimensions for page weight. The Why Sponsor row, Mobile & Player Branding cards, Premium/Main Stage/Venue media cards, orange vendor cards, and Multi-Phase Marketing Reach connector remain sponsor-scoped and responsive across desktop, tablet, and mobile. Ticket, Stripe/payment controls, Thai CSS, event pages, header/footer, DNS/SSL/cPanel/.htaccess behavior, deployment ZIP scripts, and live upload behavior remain unchanged. No dev server, preview port, cPanel ZIP, live upload, or Serena dashboard was used.
- The 2026-06-27 sponsor pill/background fix keeps `/become-a-sponsor` in the same light slide-deck design while removing the visible checker/grid background from the page surface. Sponsor category pills now use the supplied transparent white PNG icon assets where requested: magnifying glass for Event Overview, vendor booth for Vendor Program, media player for Media Inventory, and handshake for Partner Value. The uploaded icon files were cleaned so their checkerboard backplates no longer render inside pills. Multi-Phase Marketing Reach now draws the blue connector only between desktop cards, with no line crossing through cards and no connector on tablet/mobile. Food & Hospitality, Ticket, Stripe/payment controls, Thai CSS, event pages, header/footer, DNS/SSL/cPanel/.htaccess behavior, deployment ZIP scripts, and live upload behavior remain unchanged. No dev server, preview port, cPanel ZIP, live upload, or Serena dashboard was used; `WEIGHT.md` was not present.
- The 2026-06-27 sponsor responsive repair pass fixes `/become-a-sponsor` as a complete desktop/tablet/mobile layout polish without changing unrelated pages. Sponsor sections now use sponsor-scoped sizing safeguards, wrapping headers, full-width mobile cards, readable tablet two-column tracks, and desktop grids only where card text has room. Stat cards keep the reference-like light card, blue border, rounded blue top band, large blue stat, bold black title, and readable body hierarchy while switching safely from four columns on desktop to two on tablet and one on mobile. Sponsorship and Partner Value pills use a wider clean white handshake SVG on their blue/red pills, Campaign Reach keeps a clean white megaphone, and all pills have safer max-width, padding, and icon sizing so text does not clip. Campaign Reach now better matches the reference with outlined blue cards, centered numbered circles, blue headings, black bullets, and a desktop connector line, while tablet/mobile remove the connector and stack cleanly. Partner Value benefits use mobile-safe check spacing, and Mobile & Player Branding icons/cards are less bulky and more consistent. No dev server, preview port, cPanel ZIP, live upload, or Serena dashboard was used; `WEIGHT.md` was not present.
- The 2026-06-27 sponsor/Ticket refinement keeps the existing sponsor page slide-deck concept and Ticket layout intact while correcting the requested section details only. The Ticket page Thai Registration Form button now targets `https://docs.google.com/forms/d/e/1FAIpQLSdzq8BfEIQPQ5O79T7kOH-LHvzQA8q5TyXdaSRgA2ZwUg9eGQ/viewform`; English Registration, Thai Authorization, English Authorization, Stripe Buy Buttons/scripts, prices, and Ticket styling remain unchanged. Sponsor stats now read closer to the reference with a true four-column desktop row, white cards, blue top bands, large blue stat typography, bold black titles, regular body text, and safer desktop width so `Thousands` and `Brand Exposure` do not clip or wrap awkwardly. Campaign Reach now uses three light cards with thin blue borders, centered blue number badges, blue headings, black bullets, and a subtle desktop connector line. Campaign Reach, Partner Value, and Sponsorship pills use cleaner megaphone/handshake SVG icons, and Sponsorship has explicit desktop no-clip sizing. Mobile & Player Branding keeps the blue band and structure while using a more consistent, less bulky icon set and even title/body spacing. No dev server, preview port, Serena dashboard, cPanel ZIP, or live upload was used; `WEIGHT.md` was not present.
- The 2026-06-27 sponsor responsive repair keeps the sponsor page in the light slide-deck direction while fixing tablet and mobile behavior. Homepage About now uses a wider desktop/tablet heading measure for the full festival statement so it reads as an editorial block rather than a skinny mobile column, and the hero `GET TICKET NOW` button is visually anchored near the lower edge inside the video hero. `/become-a-sponsor` now has a real responsive ladder: wide desktop retains slide-like left/right layouts where space allows, 768px-1199px uses readable two-column cards or stacked image/text sections, and mobile stacks full-width cards with safe padding. Stat cards no longer collapse `3K-10K+` or long words, the four-card sponsor section becomes 2x2 on tablet and single-column on mobile, and the three-card/benefits sections stop squeezing text. Sponsor images use stable rounded frames, cover for scene photos, and contain for the premium placement collage with an image-matched aspect ratio. The old CSS icon blobs were replaced with consistent inline SVG icons using section color: search/event overview, storefront/vendor, food, gear/service, handshake/sponsorship, video/media, megaphone/campaign reach, and check/partner value. Ticket, Stripe, header/footer, Thai toggles/rendering, SEO, DNS/SSL/cPanel/.htaccess, and ZIP scripts remain unchanged; no dev server, preview server, local port, cPanel ZIP, or upload was used.
- The 2026-06-27 sponsor/homepage rebuild keeps the existing tactical site header/footer and homepage hero media intact while refining two homepage layout details: the desktop About headline block is wider for natural wrapping, and the `GET TICKET NOW` hero CTA sits near the bottom boundary inside the video hero without overlap or a black gap. `/become-a-sponsor` now follows the supplied 12-slide reference system instead of the earlier four-section draft. Sponsor sections sit directly on a light off-white paper-texture background with no giant enclosing boxes, use vertical title accent bars, top-right rounded category pills, and color phases for green Event Overview, orange Vendor Program, blue Media Inventory, and red Partner Value. Internal cards appear only where the references show cards, with off-white card faces and colored top bands. Desktop layouts spread content left/right with image/text pairings, two-card and three-card groups, a connected three-step marketing row, and a red two-column benefits list; tablet/mobile layouts stack without horizontal overflow. Cropped deploy-safe sponsor media assets are used for the gun wall, premium placement collage, and main stage/LED screen references. Typography follows the requested hierarchy: section titles at `font-weight: 800` with `letter-spacing: -0.03em`, subheads/card titles at `font-weight: 800`, and body text at `font-weight: 400` with `line-height: 1.35`. Header/footer, Ticket, Stripe, Thai toggles, SEO, DNS/SSL/cPanel/.htaccess, and deployment ZIP behavior remain unchanged; no dev server, preview server, local port, cPanel ZIP, or live upload was used.
- The 2026-06-27 sponsor/location update keeps the current header, footer, Ticket page, Stripe buttons, Thai toggles, Event Info eligibility link, Event Map section, hero video media, DNS/SSL/cPanel/.htaccess behavior, and deployment behavior unchanged. The homepage hero CTA is positioned lower inside the native video hero only. Force of Conquest location text is standardized to `Saraburi, Thailand` on the homepage event box, event detail hero/details surfaces, and English Event Info row. `/become-a-sponsor` is now a dedicated premium presentation-style page with four structured sections: Unparalleled Scope & Scale, Vendor & Commercial Zones, Food & Beverage Zone, and Retail & Airsoft Marketplace. Sponsor sections use the requested soft light background system, rounded white cards, green/orange pill callouts, green/orange accent bars, generous spacing, existing deploy-safe market and airsoft rifle imagery, and responsive desktop/tablet/mobile collapses. Sponsor typography follows the required rules: big section titles use `font-weight: 800` and `letter-spacing: -0.03em`; section headings and small card titles use `font-weight: 800`; body text uses `font-weight: 400` and `line-height: 1.35`. The existing SEO pattern is reused for the Sponsor page rather than adding a duplicate SEO system. No dev server, preview server, local port, or Serena dashboard was opened.
- The 2026-06-27 deployment-update preparation is packaging-only for the latest built site. The production build keeps the existing visual design, header/footer, Ticket/Stripe controls, Thai language styling, Event Info/Event Map/Dress Code/Eligibility/Ticket anchor behavior, Equipment/Travel/Privacy bilingual content, and Vendor/Sponsor footer placement unchanged. `mstar-airsoft-update-deployment.zip` is a changes-only cPanel overwrite package with root `index.html`, root `.htaccess`, current compiled JS/CSS assets, changed referee image, and `sitemap.xml`, all using forward-slash archive paths with no nested `dist/` or project folder and no Windows backslash entries.
- The 2026-06-27 sponsor/footer adjustment keeps the existing footer column design and unboxed vendor page layout unchanged while moving the visible `Become a Sponsor` navigation entry to the footer directly below `Become a Vendor`. The vendor page no longer displays the separate underlined `Become a Sponsor` text link beneath the vendor application button. The sponsor route/page remains available, the vendor Google Form CTA remains unchanged, and no header, Stripe, cPanel, DNS, SSL, or deployment behavior changed.
- The 2026-06-27 ticket/language fix keeps the existing tactical page layouts, Stripe payment surfaces, header/footer, event media, DNS/SSL/cPanel behavior, and deployment behavior unchanged. `/ticket` now shows the camping add-on as `Camping Package` and no longer displays the separate `Camping Extras / Add-Ons` heading above it. Terms & Conditions and other info-page language toggles reset cleanly to the default page language on route changes and allow repeated English/Thai switching with correct active states. Thai content now uses Thai-safe font handling across `[lang="th"]`, policy pages, Event Info Thai mode, and Mission Scenario Thai mode: Noto Sans Thai/Prompt/Inter stack, normal letter spacing, no forced uppercase, normal Thai word breaking, and comfortable line height for mobile, tablet, and desktop rendering.
- The safe cPanel ZIP workflow must create Linux/cPanel-safe archive paths with forward slashes, such as `assets/index-CXNzadvn.js`, never Windows backslash entries like `assets\index-CXNzadvn.js`. The deployment ZIP root must contain `index.html` and `.htaccess` directly, with real `assets/`, `images/`, `banners/`, and `videos/` folders and no nested `dist/` or project folder. Use `npm run zip:deploy` to generate `mstar-airsoft-live-deployment.zip` from the current `dist` contents; this is a packaging safety update only and does not change visual design, header/footer, Ticket/Stripe controls, or page layouts.
- The 2026-06-27 white-homepage fix was deployment/package related, not a visual redesign: live `index.html` was present, but the referenced live JS/CSS asset URLs returned the SPA fallback as HTML because the matching compiled assets were missing from the server. The fresh full cPanel ZIP keeps the existing design unchanged and packages the complete `dist` root with `index.html`, `.htaccess`, `assets/`, `images/`, and `videos/` together so the homepage and inner pages can hydrate normally. Header/footer, Ticket/Stripe controls, page layouts, image crops, GitHub Pages base behavior, DNS, SSL, and cPanel settings were not changed.
- The 2026-06-27 event/legal/travel update keeps the current tactical visual system and unboxed legal-guide layout. `/privacy` and `/how-to-get-to-the-event` now support Thai language buttons while the existing English sections remain the default view; Thai pages use the existing Thai font treatment. The Event Info important-information image now uses the supplied referee briefing PNG without changing the image/text split layout, and the old unreferenced briefing JFIF was removed from deploy paths. Event detail/Event Info waiver links use restrained gold underlined text to reach the Ticket authorization anchor, Accommodation links directly to the Ticket camping anchor, and `/become-a-sponsor` plus the vendor-page sponsor text link sit directly on the dark background without a new card or panel. No header/footer, Stripe/payment layout, event map sizing, Mission Scenario, Equipment, Dress Code copy, countdown layout, cPanel ZIP, live upload, or dev server changed.
- The Git/source-verified Hostinger package keeps the visual design unchanged while tightening deploy contents. The final ZIP includes compiled app assets and only source-referenced deploy media from the Git/current working tree: `assets/`, `banners/`, `images/`, and `videos/` at the root, with no unused `gallery/` copy, no old deploy folders, no source folders, no docs, and no source maps. Browser verification against the pruned staging folder confirms the same rendered pages, Thai toggles, and media behavior before packaging.
- The runtime-verified Hostinger packaging pass keeps the visible design unchanged while strengthening delivery checks: `public/.htaccess` now includes explicit UTF-8 MIME `AddType` rules, and source copy was normalized away from risky smart punctuation, en/em dashes, and euro glyphs where those appeared in visible site content. The temporary Playwright verifier confirms desktop/mobile rendered DOM text, Thai language toggles, and local media references from built `dist` before packaging. No layout, image crop, video presentation, route behavior, Stripe/payment behavior, ticket pricing, forms, or live deployment changed.
- The Hostinger text/media fix packaging pass adds internal design token-saving rules only: avoid duplicated UI/content blocks, avoid large inline assets, prefer reusable components, keep repeated page copy concise, and avoid redundant explanatory UI text. No visible design, layout, Stripe behavior, image artwork, video artwork, page structure, cPanel/Hostinger settings, live upload, or deployment changed.
- The Hostinger text/media fix keeps the existing visual design and separate Vite runtime folders unchanged while improving deployment safety: `public/.htaccess` now declares UTF-8 for HTML/CSS/JS/JSON/XML/TXT/SVG responses, local media references remain under their generated `/images/`, `/videos/`, and `/assets/` paths, and `/privacy-policy` is a compatibility alias to the existing Privacy page for the live checklist URL. No Stripe behavior, forms, prices, page layout, image/video artwork, GitHub Pages deployment, live upload, or Hostinger settings changed.
- Phase 2 optimization keeps the homepage hero visually and behaviorally unchanged while replacing the active `force-of-conquest-header-compress-video.mp4` with a same-path optimized MP4. The original is backed up outside deployed paths, the hero keeps poster fallback, muted autoplay, loop, playsInline, no controls, crop/overlay/CTA behavior, and the current build output is smaller without a cPanel ZIP or real server deployment.
- Phase 1 deploy bloat cleanup moved unused heavy videos out of deployed `public/` paths into `site content/non-deployed-heavy-media-backup/` after local reference checks and read-only live production Network/DOM checks confirmed they were not used. The live homepage hero remains `force-of-conquest-header-compress-video.mp4`; no visible design, routing, layout, cPanel ZIP, or real server deployment changed.
- The bilingual Event Info `Important Information Before Purchasing` section uses the supplied referee/event staff briefing photograph in the existing responsive image/text split. The centered `object-fit: cover` treatment keeps the high-visibility referee visible without distortion or awkward cropping, while the text remains directly on the tactical page background with no added box. No cPanel ZIP was created and no real server upload was performed.
- The English Event Info Dress Code copy uses a welcoming, encouraging tone: exact small uniform details are not enforced strictly, while faction dress is encouraged for immersion, realism, team recognition, authenticity, atmosphere, and fun. Existing faction camouflage banners and logos remain unchanged. The Event Information Eligibility row keeps the existing text-only row styling and adds a restrained gold underlined `Click here` link for participants aged 17 and younger, routing to the Ticket page’s stable `authorization-of-supervising-adult` anchor. The anchor uses scroll offset for the sticky header and does not alter Ticket form buttons or Stripe controls. No cPanel ZIP was created and no real server upload was performed.
- Force of Conquest Event Info includes a bilingual Event Map section directly below Event Information. It uses the updated uploaded `map final edit.png` image, copied as `force-of-conquest-event-map-updated.png`, and the display is now reduced on desktop with a centered `width: min(100%, 980px)` treatment. The map keeps automatic height, tablet/mobile responsiveness, and direct dark-background placement without a card, panel, heavy border, cropping, stretching, or horizontal overflow. No cPanel ZIP was created and no real server upload was performed.
- The Become a Vendor page must stay unboxed: the eyebrow, `BECOME A VENDOR` title, `STALLHOLDER APPLICATION FORM` heading, paragraph copy, and gold Google Form CTA sit directly on the dark tactical page background with spacing and a subtle gold divider instead of a large card, bordered panel, or dark rectangle behind all content. The vendor Google Form URL remains unchanged, mobile width stays constrained, and no cPanel ZIP or real server upload was performed.
- SEO coverage was audited across current public routes. The existing lightweight system remains the source of truth: `SeoHead`, route-level metadata in `src/data/seoContent.ts`, live-domain canonicals, meta descriptions, Open Graph, Twitter cards, JSON-LD, route aliases, and sitemap route data. Static `public/sitemap.xml` should stay aligned with the current public route list, including Force of Conquest detail, Event Info, Mission Scenario, Become a Vendor, and Cancellation and Refund.
- Phase 3 image optimization keeps visible design, layout, crop behavior, routes, buttons, Stripe, header, footer, cPanel settings, and deployment behavior unchanged while reducing deployed image weight. The Event Info map remains a same-path optimized PNG with readable labels and no crop/layout change. The Force of Conquest card remains a same-path optimized PNG so event-card and SEO/social references still work. Kaeng Khoi, Kayaking Rapids, Siam Frontier Coalition, and Golden Triangle Syndicate images now use deploy-safe WebP paths from typed content/config, with visual quality checked on the relevant desktop/mobile built routes. No cPanel ZIP or real server upload was performed.
...
## Design Token-Saving Rules

- Avoid duplicating UI/content blocks across pages when shared data or reusable components can preserve the same presentation.
- Do not embed large inline images, videos, SVG payloads, or base64 media in components or content files.
- Keep repeated page copy concise and structured so future edits stay data-driven and easy to verify.
- Prefer reusable section components for repeated cards, legal text layouts, guide blocks, event media rows, and CTAs.
- Do not create redundant explanatory text blocks in the UI when navigation labels, headings, and concise body copy already explain the page.
...
- Keep the Serena dashboard disabled unless a task specifically requires its interface.
- Use the configured project scripts and existing default/bundled Python runtime without interrupting ordinary design work to ask the user which app or interpreter to use.
...
- 2026-06-25: Continued the Event Info and Equipment refinement after the required memory-file read, using `$caveman full` and token-saving behavior while skipping every previously completed item. Removed the standalone Age Requirement visual section in both Event Info languages but retained eligibility within the Event Information rows. The Event Info introduction now uses the supplied helmet-mounted camera image as an optimized local WebP with responsive cover presentation. Mandatory Dress Code is now an intentional full-width faction uniform composition instead of a placeholder: two equal tactical pattern panels sit side by side on desktop and stack on mobile, with strong white faction names, gold uniform labels, dark readability gradients, and supplied woodland/tiger-stripe textures. The Equipment page now uses the existing unboxed policy layout and English/Thai toggle to present the complete transportation and security protocol as eleven readable sections per language, including registration, air travel, Thai arrival inspection, security seals, storage, authorized use, export, and zero-tolerance rules. Desktop and 390px mobile checks confirmed natural heading wrapping, clear pattern labels, no horizontal overflow, no framework overlay, and unchanged Ticket/Stripe rendering. No cPanel ZIP or real server deployment was created; the Serena dashboard was not launched and no unnecessary Python/app prompt was shown.

- 2026-06-25: Began with the required `AGENT.md` / `DESIGNER.md` read and `$caveman full` token-saving workflow. Refined the bilingual Force of Conquest Event Info composition while preserving its open dark editorial direction and all existing navigation/payment behavior. The ticket-inclusions split section retains its media footprint but presents the full event poster with centered `object-fit: contain` treatment and restrained internal breathing room. Event Information and Camping Rules now use intentional text-only layouts with no empty media boxes; Camping Rules sits closer to Camping Information to create one readable camping sequence while retaining a clean gold divider. The LED armband H2 uses a smaller responsive scale and wider measure for balanced wrapping. Facilities now uses the supplied night-market food-vendor scene, Camping uses the supplied tent scene, Experience Realistic Military Simulation uses the supplied ruined-building team scene, and Sanitary Facilities uses the supplied portable-toilet scene. All new bitmap assets were optimized to local WebP files, remain cover-fit where requested, and load through deploy-safe public paths. Desktop 1440px and mobile 390px rendered checks confirmed loaded imagery, English/Thai switching, no clipping, no horizontal overflow, and no framework overlay. No cPanel ZIP or real server deployment was created.

- 2026-06-25: Reworked the Event Info page’s split-section system from bordered dark cards into an open cinematic editorial composition. Images retain their own cover-fit media areas, while all text now sits directly on the dark tactical page background with generous gutters and thin gold top rules instead of rectangular panel framing. The page headline beneath `EVENT INFO` is bold gold/yellow. Repeated title/headline/date copy was removed from the first content section, which now uses `c7ddb663-e7b8-47e9-8063-e4e8730a1ffb.jfif`; the realistic MilSim section now uses `874d0ed7-e484-4b41-964e-18b9aebd6a28.jfif`. Removed the closing tactical slogan from that section. All Event Info H2 headings now share a restrained `clamp()` scale, normal word wrapping, safe line height, and wider text measure across desktop/tablet/mobile. Removed the old image-led Join the Battle section in both languages and replaced it with the unboxed event-detail CTA pattern and gold Buy Ticket Now button. English/Thai language controls and Thai-safe typography remain unchanged. The work followed token-saving and automatic commit/push rules; no cPanel ZIP or real server upload was created.

- 2026-06-24: Added the Force of Conquest `SEE EVENT INFO` experience. A restrained-height full-bleed image banner now follows the Overview / Operation Brief area, uses the locally stored Specna Arms `ck-UftftEGs` photograph with a dark tactical readability overlay, contains only the title directly over the image, and routes as one large click target to `/events/force-of-conquest/event-info`. The new bilingual Event Info page uses English/Thai tactical language controls and large alternating image/text panels on desktop that stack media-first on tablet/mobile. Its visual system uses dark charcoal/forest panels, thin gold rules, white tactical headings, muted body copy, local approved imagery, purposeful `Image Coming Soon` placeholders, cover-fit media, Thai-safe fonts, and overflow-safe content rows. English content comes from `3 Days • 2 Nights of Non-Stop Action, Adventure, and Immersion.pdf`; Thai content comes from `3 วัน 2 คืน แห่งการต่อสู้ ผจญภัย และประสบการณ.pdf` with only extraction spacing repaired. The existing Mission Scenario banner/page and unboxed Choose Your Factions section remain visually unchanged. The implementation stays typed, scoped, responsive, token-conscious, and included the automatic commit/push workflow; no cPanel ZIP or real server deployment was created.
...
- 2026-06-24: Added tactical English/Thai language buttons to the Mission Scenario page with English active by default and Thai content added from `เนื้อเรื่องภารกิจหลักตลอด 3 วัน(1).pdf` / the local Thai storyline PDF. Both languages render as normal responsive content on the unboxed dark tactical page, with the Option 3 hierarchy preserved and Thai body copy using the `"Noto Sans Thai", "Prompt", "Inter", Arial, sans-serif` stack. Added a new Choose Your Factions section before the Mission Scenario banner on the Force of Conquest event detail page. The section uses two side-by-side desktop faction cards that stack on mobile, centers responsive `object-fit: contain` logos, and features Siam Frontier Coalition from `team 2.png` and Golden Triangle Syndicate from `team 1.png` via deploy-safe public copies `team-2.png` and `team-1.png`. The Mission Scenario banner remains after the faction section with the same tactical image treatment, `VIEW FULL SCENARIO` button, and route to `/events/force-of-conquest/mission-scenario`. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-24: Replaced the Mission Scenario page text using the new PDF source of truth, `Three-Day Campaign Storyline FORCE OF CONQUEST(1).pdf`, and updated the Force of Conquest 2027 Three-Day Campaign Storyline content for Background, Day 1, Day 2, Day 3, Night Festival & Entertainment, and The War Never Stops. The page keeps the Option 3 typography system with large gold Oswald DAY labels, white Barlow Condensed operation titles, Rajdhani section headings, Inter body copy, JetBrains Mono schedule times, thin gold dividers, unboxed dark tactical layout, readable 900px-1100px content width, and mobile-safe stacked schedule rows. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-24: Updated the Force of Conquest event detail page and cards. The Mission Scenario banner now keeps only the eyebrow, `SEE MISSION SCENARIO` heading, and visible `VIEW FULL SCENARIO` button, removing the small `View the full three-day campaign storyline.` subtext while keeping the image-based controlled-height banner. The hero/card status now reads `REGISTRATION NOW OPEN!`, the Draft Event Flow / Schedule Timeline section was removed from the event detail page so the fuller storyline stays on the Mission Scenario page, Force of Conquest event cards now show `€120 Pre Sale`, and TBA event cards now show price/entry fee as `TBA`. The homepage Force of Conquest card now displays the same approved event image as the detail page, while TBA cards remain static and without dedicated pages. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-24: Refined the Force of Conquest event detail page presentation by removing boxed/bordered badge styling from the top hero event labels so the date, location, and registration status now read as unboxed tactical inline text. The `Ready for Force of Conquest?` final CTA no longer uses a boxed or panel-like background and now relies on open dark-page spacing, a thin divider, and the existing gold Buy Ticket Now button. The Mission Scenario banner keeps its cinematic image background and controlled height, now has a larger gold Rajdhani/Barlow-style eyebrow, and includes a visible `VIEW FULL SCENARIO` button that routes with the banner to `/events/force-of-conquest/mission-scenario`. Countdown remains unboxed in the clean row layout. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-24: Applied the Option 3 font hierarchy to the dedicated Mission Scenario page at `/events/force-of-conquest/mission-scenario`. DAY labels now read as larger gold Oswald-style tactical chapter markers with uppercase letter spacing and no box treatment. Operation titles use the Barlow Condensed stack, operation subtitles and mission/story section headings use Rajdhani-style condensed uppercase typography with thin gold divider lines, body copy uses Inter, and schedule time rows use JetBrains Mono for a technical event-ops feel. The Mission Scenario story remains unboxed directly on the dark tactical page background with readable 900px-1100px content width and mobile-safe schedule stacking. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-24: Replaced the long Force of Conquest Mission Scenario section on the event detail page with a short controlled-height clickable banner. The banner uses the existing mission scenario jungle image as a cinematic background, avoids the previous oversized/tall layout, keeps text centered and readable without a boxed card, and links internally to `/events/force-of-conquest/mission-scenario`. Added the dedicated Mission Scenario page with the full Three-Day Campaign Storyline converted from the PDFs into responsive web content. The page uses a readable centered content width, open dark tactical background, strong white headings, muted body text, gold accents, section dividers, clean schedule rows, checklist rows, and mobile-safe stacking so text is not squeezed into narrow vertical columns. The countdown remains a clean unboxed separator row, Force of Conquest routing remains `/events/force-of-conquest`, and TBA cards remain static with no dedicated pages. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-24: Refined the Force of Conquest event detail page so primary text content now sits directly on the dark tactical page background without large boxed/card panels. The page relies on modern cinematic spacing, strong white headings, muted body copy, gold and military-green accents, thin dividers, and open section rhythm instead of dashboard/table styling. Event Media/Gallery placeholders were replaced with a full-bleed Mission Scenario section using `public/images/events/force-of-conquest-mission-scenario.jfif`, copied from the supplied `bf63be0a-13f2-4ad0-96d0-e99babac29e5.jfif`, as a viewport-wide jungle/milsim background with a readable cinematic overlay. Mission Scenario content comes from the uploaded Three-Day Campaign Storyline PDFs and covers Force of Conquest 2027, the Republic of Siam Frontier conflict, Golden Triangle syndicate control, faction strategy/leadership/diplomacy/combat, Day 1 Operation Jungle Storm, Day 2 Operation Golden Triangle, Day 3 Operation Final Conquest, Night Festival & Entertainment, and The War Never Stops. The homepage countdown was restyled into a compact clean row with colon separators, large white numbers, small labels, and no individual number boxes. Force of Conquest routing remains `/events/force-of-conquest`; TBA cards remain static without dedicated event pages. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-23: Added a live tactical homepage countdown above Game Schedules for the Force of Conquest gate date, January 8, 2027. The countdown uses browser time, updates every second, clears its interval on unmount, and sits inside the existing Game Schedules background system so the timer, heading, and event cards feel connected rather than separated by a hard border. Desktop uses a bold horizontal count, while tablet/mobile can wrap into compact columns without clipped labels or horizontal overflow.

- 2026-06-23: Added a dedicated Force of Conquest event detail page at `/events/force-of-conquest` and changed Force of Conquest cards on both the homepage and Events/Operations page to open that page. The TBA event cards remain static with no dedicated pages yet. The event detail page follows the uploaded tactical UI/UX reference direction for cinematic flow, strong dark military sections, white headings, gold CTAs, green accents, structured detail rows, timeline cards, requirement checklist styling, and intentional placeholder media blocks for future images without copying another brand.

- 2026-06-23: Kept Events page wording in the requested Operations direction with `ALL OPERATIONS` removed. The new Force detail page keeps Buy Ticket Now routed to `/ticket` and View Rules routed to `/rules-and-regulation`, preserves responsive desktop/tablet/mobile behavior, and leaves Ticket pricing, Stripe Buy Button IDs, Stripe wrappers, Stripe script behavior, Ticket payment layout, header, footer, Terms, Cancellation and Refund, DNS/SSL/cPanel settings, `.htaccess`, existing video links, and registration form links unchanged. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-23: Updated the Ticket page Early Bird / Pre Sale Stripe Buy Button presentation so the visible Stripe host reads as a pill-shaped payment area instead of a square box. The scoped package-button treatment keeps the button centered, capped at the existing 360px max width, clipped with `border-radius: 999px`, and responsive across desktop, tablet, and 360/390/430px mobile widths. Early Bird / Pre Sale keeps Buy Button ID `buy_btn_1TiUFN5Kev5Ia2RiD8wrJaVz`, Stripe script loading remains single-instance, payment behavior is still handled by Stripe, and the Camping Experience add-on remains unchanged. No cPanel ZIP was created and no real server upload was performed.

- 2026-06-23: Clarified the project-facing homepage links so the GitHub Pages test homepage is visibly separate from the live production domain. README now presents `https://cynicalfocus123.github.io/Mstar-Airsoft-Website/` as the repository test preview and `https://mstarairsoft.com/` as the live production site. `package.json` homepage/repository metadata now points to the GitHub Pages test preview/repo instead of implying the live domain is the repo homepage. This was documentation/metadata only; no visual design, routes, cPanel ZIP, or live server deployment changed.

- 2026-06-23: Refined the homepage Thai/English language video section background so the existing image is brighter and more visible while preserving a dark cinematic tactical tone. The current Thai/English video cards keep their widths, heights, labels, gold-border feel, YouTube URLs, iframe behavior, and responsive layout. Homepage section transitions were cleaned up by removing hard border seams and using softer gradient edges between the splash banner, language/video section, Game Schedules, and Game Terrain so the homepage reads as a more continuous premium flow. Events page presentation now removes the `All Operations` eyebrow and changes the visible page wording from `Events` to `Operations`. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-23: Added dark cinematic homepage background image treatments to the Game Schedules and Thai/English language video sections only. Game Schedules now uses `photo-1666873577061-26f78e7452ce.avif` from `public/images/home/` with layered black/green/gold overlays so the image is visible but controlled; the existing heading, View All Events button, event cards, card widths/heights, spacing, Force of Conquest link behavior, and TBA static-card behavior were preserved. The Thai/English video section now uses `photo-1661339051428-1af0c377a793.avif` from `public/images/home/` with a dark tactical overlay; current video card widths/heights, gold-border card feel, labels, YouTube links, iframe behavior, and responsive layout were preserved. No cPanel ZIP was created and no real server upload was performed; changes were committed and pushed to GitHub for review before real server deployment.

- 2026-06-23: Added a footer-only `Cancellation and Refund` link and a new `/cancellation-and-refund` legal page. The page follows the same unboxed Terms & Conditions layout direction, keeps policy text directly on the dark tactical background without large cards or bordered text boxes, and uses two clear tactical English/Thai language buttons with an active state. English content was added from `EVENT CANCELLATION, REFUND _ TRANSFER POLICY.pdf`, Thai content was added from `นโยบายการยกเลิก การคืนเงิน และการโอนสิทธิ.pdf`, and no PDF iframe/viewer was used. Header, Ticket pricing, Stripe controls, cPanel ZIP/server deployment, DNS/SSL/cPanel settings, and `.htaccess` were not changed; changes were committed and pushed to GitHub for review before real server deployment.
...
- 2026-07-06: Added the Ticket required-forms Waiver entry through the existing form-section layout so it inherits the current dark panel, green eyebrow, white title, gold button, and responsive behavior.
